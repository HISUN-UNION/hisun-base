 ...
 * 
 *<p>类名称：BaseDaoImpl</p>
 *<p>类描述: 实现了一些常用的数据访问方法，包括实体对象的增加、修改、删除、列表查询、条件查询等。
 *各具体业务模块数据访问实现通过继承此基类后，不再需要编写常用的数据访问接口方法。</p>
    ...
    public Session getSession() {
    	...
    	if(TenantEntity.class.isAssignableFrom(this.entityClass)){    		
    		...
    		try{
    			...
    			userLoginDetails = (UserLoginDetails) SecurityUtils.getSubject().getSession().getAttribute(Constants.CURRENT_USER);
